...
class DoWhileExp : public Exp {
 ...
 public:
  DoWhileExp(unique_ptr<Exp> body, unique_ptr<Exp> test)
      : body_(move(body)), test_(move(test)) {}
  Value *codegen(CodeGenContext &context) override;
...
class ForExp : public Exp {
 ...
 public:
  ForExp(int first_line, int first_column, string var, unique_ptr<Exp> low, unique_ptr<Exp> high,
         unique_ptr<Exp> body)
